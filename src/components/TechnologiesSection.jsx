...
const TechnologiesSection = () => {
  return (
    <section aria-labelledby="technologies-heading" className="mt-12">
      ...
      <div className="mt-6 rounded-3xl border border-white/5 bg-[#0f0f0f]/40 p-4 shadow-[0_20px_50px_rgba(0,0,0,0.4)] backdrop-blur-md overflow-hidden">
        {/* Marquee wrapper */}
          ...
          <div className="marquee-track flex items-center gap-6 whitespace-nowrap will-change-transform animate-marquee">
            {technologies.concat(technologies).map(({ name, Icon, className }, idx) => (
              <div key={`${name}-${idx}`} className="inline-flex items-center justify-center">
                <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-white/5 border border-white/5 p-3.5 transition-all duration-300 transform-gpu opacity-80 hover:opacity-100 hover:scale-110 hover:border-orange-500/30">
